...
export function titleForProofDate(dateKey) {
  const reviewMatch = String(dateKey || "").match(/^\d{4}-(\d{2})-확인필요$/);
  if (reviewMatch) {
    return `${Number(reviewMatch[1])}월`;
  }
...
export function groupProofImagesByDate(images, monthKey = "") {
  ...
  for (const image of images || []) {
    ...
    if (!groups.has(dateKey)) {
      groups.set(dateKey, { dateKey, route: [], oil: [], toll: [], extra: [], welfare: [], supply: [], other: [], review: [] });
    }
    const bucket = groups.get(dateKey);
    const type = ["route", "oil", "toll", "extra", "welfare", "supply", "other", "review"].includes(image.type) ? image.type : "extra";
