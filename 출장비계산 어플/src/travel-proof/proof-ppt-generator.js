...
export async function buildProofPptxBuffer({ monthKey, images }) {
  ...
  for (const proofSection of proofPresentationSections(grouped)) {
    pptx.addSection?.({ title: proofSection.title });
    for (const item of proofSection.items) {
      if (item.kind === "field") {
        addProofSlide(pptx, item.group, proofSection.title);
      } else {
        addExpenseProofSlide(pptx, item.group, item.section, proofSection.title);
      }
    }
  }
...
function proofPresentationSections(grouped) {
  const sections = [
    { title: "1. 현장지원", items: [] },
    { title: "2. 일반출장", items: [] },
    { title: "3. 조직활성화비, 소모품비", items: [] },
    { title: "4. 기타", items: [] }
  ];
  for (const group of grouped) {
    if ([...(group.route || []), ...(group.oil || []), ...(group.toll || [])].length) {
      sections[0].items.push({ kind: "field", group });
    }
    if ((group.extra || []).length) {
      sections[1].items.push({
        kind: "expense",
        group,
        section: { key: "extra", label: "일반출장 증빙", images: group.extra || [] }
      });
    }
    for (const section of expenseProofSections(group)) {
      const target = section.key === "review" ? sections[3] : sections[2];
      target.items.push({ kind: "expense", group, section });
    }
  }
...
function expenseProofSections(group) {
  return [
    ...
    { key: "other", label: "기타 증빙", images: group.other || [] },
    { key: "review", label: "확인필요 증빙", images: group.review || [] }
  ].filter((section) => section.images.length);
}

function addProofSlide(pptx, group, sectionTitle) {
  const slide = pptx.addSlide({ sectionTitle });
...
function addExpenseProofSlide(pptx, group, section, sectionTitle) {
  const slide = pptx.addSlide({ sectionTitle });
  slide.background = { color: "FFFFFF" };
  slide.addShape(pptx.ShapeType.rect, {
    x: 0.8,
