...
test("ppt output naming and slide title use the selected month and date only", () => {
  ...
  assert.equal(titleForProofDate("2026-05-확인필요"), "5월");
});
...
test("groupProofImagesByDate keeps toll proof separate from extra proof", () => {
  const grouped = groupProofImagesByDate([
    { type: "toll", name: "통행료캡처/toll-2026-05-19.png", dateKey: "2026-05-19" }
  ], "2026-05");

  assert.equal(grouped[0].toll.length, 1);
  assert.equal(grouped[0].extra.length, 0);
});

test("groupProofImagesByDate keeps external receipt photos on a monthly review page", () => {
  const grouped = groupProofImagesByDate([
    { type: "review", name: "배정환/6월/영수증.jpg", dateKey: "2026-06-확인필요" }
  ], "2026-06");

  assert.equal(grouped.length, 1);
  assert.equal(grouped[0].dateKey, "2026-06-확인필요");
  assert.equal(grouped[0].review.length, 1);
});
